...
const defaultJasaDb = [
  { kode: "Tidak Ada", harga: 0 },
  { kode: "LGGC", harga: 180930 },
  { kode: "LCGC", harga: 180930 },
  { kode: "NEW ENTRY", harga: 180930 },
  { kode: "COM", harga: 223110 },
  { kode: "ECO", harga: 256410 },
  { kode: "PAS", harga: 329670 },
  { kode: "MEXL", harga: 567210 },
  { kode: "HEXL", harga: 643800 },
  { kode: "CBU", harga: 643800 },
  { kode: "LEXUS", harga: 705960 },
];
...
const emptyForm = {
  polisi: "", kendaraan: "", rangka: "", tahun: "", customer: "", contact: "", phone: "", alamat: "", catatan: "",
...
export default function App() {
  ...
  const [jasaDb, setJasaDb] = useState(() => load("jasa_db_v5", defaultJasaDb));
  const [repairType, setRepairType] = useState("");

  const flatRateList = useMemo(() => {
    const obj = {};
    (jasaDb && jasaDb.length ? jasaDb : defaultJasaDb).forEach((x) => {
      if (!x.kode) return;
      obj[x.kode] = Number(x.harga) || 0;
    });
    if (obj["Tidak Ada"] === undefined) obj["Tidak Ada"] = 0;
    return obj;
  }, [jasaDb]);
  const flatRateOptions = useMemo(() => Object.keys(flatRateList), [flatRateList]);
  ...
  useEffect(() => save("jasa_db_v5", jasaDb), [jasaDb]);
  ...
  useEffect(() => {
    ...
    const data = {
      ...
      customer: form.customer, contact: form.contact, phone: form.phone, alamat: form.alamat, catatan: form.catatan,
    };
    setCustomerDb((prev) => {
  ...
  const updateHargaSpareJasa = () => {
    setForm((prev) => ({
      ...prev,
      parts: prev.parts.map((r) => {
        const found = partDb.find((x) => norm(x.partNo) === norm(r.partNo));
        const price = found ? Number(found.price) || 0 : Number(r.price) || 0;
        const qty = Number(r.qty) || 0;
        return { ...r, price, total: qty * price, partName: found?.partName || r.partName };
      }),
      jasa: prev.jasa.map((r) => {
        const flatPrice = flatRateList[repairType] || Number(r.price) || 0;
        const rate = toDecimal(r.rate);
        return { ...r, price: flatPrice, total: rate * flatPrice };
      }),
    }));
    alert("Harga sparepart dan jasa sudah diupdate dari master/flat rate.");
  };
  ...
  return <div onPaste={handlePaste} style={styles.app}>
    <aside style={styles.sidebar}><div style={styles.logo}>WS</div><div style={styles.nav}>⌂ Home</div><div style={styles.navActive}>⚙ Estimasi ›</div><button style={page==="estimasi"?styles.menuOn:styles.menu} onClick={()=>setPage("estimasi")}>▣ Buat Estimasi</button><button style={page==="history"?styles.menuOn:styles.menu} onClick={()=>setPage("history")}>↻ History Estimasi</button><button style={page==="sparepart"?styles.menuOn:styles.menu} onClick={()=>setPage("sparepart")}>🔧 Master Sparepart</button><button style={page==="masterjasa"?styles.menuOn:styles.menu} onClick={()=>setPage("masterjasa")}>🔨 Master Jasa</button><button style={page==="customer"?styles.menuOn:styles.menu} onClick={()=>setPage("customer")}>👤 Master Customer</button></aside>
    <main style={styles.main}>{page==="estimasi" && <Estimasi form={form} setField={setField} addPart={addPart} updateRow={updateRow} deleteRow={deleteRow} addSublet={addSublet} addJasa={addJasa} totals={totals} repairType={repairType} setRepairType={setRepairType} handleFlatRateChange={handleFlatRateChange} partDb={partDb} customerDb={customerDb} flatRateList={flatRateList} flatRateOptions={flatRateOptions} findCustomerByPlate={findCustomerByPlate} saveMasterFromCurrent={saveMasterFromCurrent} saveHistory={saveHistory} updateHargaSpareJasa={updateHargaSpareJasa} downloadPDF={downloadPDF} resetForm={resetForm} filePartRef={filePartRef} addPartsFromImage={addPartsFromImage} pdfRef={pdfRef}/>} {page==="history" && <History history={history} editHistory={editHistory} setHistory={setHistory} newForm={() => {resetForm(); setPage("estimasi")}}/>} {page==="sparepart" && <MasterPart partDb={partDb} setPartDb={setPartDb}/>} {page==="masterjasa" && <MasterJasa jasaDb={jasaDb} setJasaDb={setJasaDb}/>} {page==="customer" && <MasterCustomer customerDb={customerDb} setCustomerDb={setCustomerDb}/>}</main>
  </div>;
}

function Estimasi(p){ const f=p.form; const customers = p.customerDb || []; return <><section style={styles.card}><h2 style={{textAlign:"center"}}>FORM ESTIMASI</h2><div style={styles.tabs}><b>Profile</b><b>Estimasi Biaya</b></div><h3>🚙 Informasi Kendaraan</h3><div style={styles.grid4}><SearchInput label="No Polisi *" value={f.polisi} suggestions={customers.map((c)=>({value:c.polisi||"",label:`${c.polisi || ""} - ${c.customer || ""} - ${c.kendaraan || ""}`}))} onChange={(v)=>{ const plat = String(v || "").toUpperCase(); p.setField("polisi", plat); p.findCustomerByPlate(plat); }} onSelect={(v)=>{ const plat = String(v || "").toUpperCase(); p.setField("polisi", plat); p.findCustomerByPlate(plat); }}/><SearchInput label="Type Kendaraan *" value={f.kendaraan} suggestions={[...new Set(customers.map(c=>c.kendaraan).filter(Boolean))].map((x)=>({value:x,label:x}))} onChange={(v)=>p.setField("kendaraan",v)} onSelect={(v)=>p.setField("kendaraan",v)}/><SearchInput label="No Rangka *" value={f.rangka} suggestions={customers.map((c)=>({value:c.rangka||"",label:`${c.rangka || ""} - ${c.polisi || ""} - ${c.customer || ""}`}))} onChange={(v)=>p.setField("rangka",v)} onSelect={(v)=>p.setField("rangka",v)}/><Input label="Tahun *" value={f.tahun} onChange={(v)=>p.setField("tahun",v)}/></div><h3>👤 Informasi Pelanggan</h3><div style={styles.grid2}><Input label="Nama Pelanggan *" value={f.customer} onChange={(v)=>p.setField("customer",v)}/><Input label="No Telepon" value={f.phone} onChange={(v)=>p.setField("phone",v)}/><Input label="Contact Person" value={f.contact} onChange={(v)=>p.setField("contact",v)}/><Input label="Alamat" value={f.alamat} onChange={(v)=>p.setField("alamat",v)}/></div><label style={{...styles.label,display:"block",marginTop:14}}>Catatan / Keterangan Tambahan<textarea value={f.catatan || ""} onChange={(e)=>p.setField("catatan",e.target.value)} style={styles.noteInput} placeholder="Ketik catatan yang ingin muncul di kolom kosong PDF..."/></label><h3>▣ Informasi Pembuat Estimasi</h3><div style={styles.grid3}><Input label="Nama Staff *" value={f.staff} onChange={(v)=>p.setField("staff",v)}/><Input label="Jabatan" value={f.jabatan} onChange={(v)=>p.setField("jabatan",v)}/><Input label="Kepala Bengkel" value={f.kepalaBengkel} onChange={(v)=>p.setField("kepalaBengkel",v)}/></div></section>
  <section style={styles.card}><h3>🔧 Estimasi Parts</h3><table style={styles.table}><thead><tr><th>FREE</th><th>PART NO</th><th>PART NAME</th><th>QTY</th><th>PRICE</th><th>TOTAL</th><th>ACT</th></tr></thead><tbody>{f.parts.map(r=><tr key={r.id}><td><input type="checkbox" checked={r.free} onChange={(e)=>p.updateRow("parts",r.id,"free",e.target.checked)}/></td><td><SearchCell value={r.partNo} suggestions={p.partDb.map(x=>({value:x.partNo,label:`${x.partNo} - ${x.partName} - Rp ${rupiah(x.price)}`}))} onChange={(v)=>p.updateRow("parts",r.id,"partNo",v)} onSelect={(v)=>p.updateRow("parts",r.id,"partNo",v)}/></td><td><SearchCell value={r.partName} suggestions={p.partDb.map(x=>({value:x.partName,label:`${x.partName} - ${x.partNo} - Rp ${rupiah(x.price)}`}))} onChange={(v)=>p.updateRow("parts",r.id,"partName",v)} onSelect={(v)=>p.updateRow("parts",r.id,"partName",v)}/></td><td><input type="number" value={r.qty} onChange={(e)=>p.updateRow("parts",r.id,"qty",e.target.value)} style={styles.smallInput}/></td><td><input value={r.price} onChange={(e)=>p.updateRow("parts",r.id,"price",toNumber(e.target.value))} style={styles.cellInput}/></td><td><b>{rupiah(r.total)}</b></td><td><button onClick={()=>p.deleteRow("parts",r.id)} style={styles.del}>🗑</button></td></tr>)}</tbody></table><button onClick={p.addPart} style={styles.btn}>⊕ Add Part</button><button onClick={()=>p.filePartRef.current.click()} style={styles.green}>♻ Add from Image / Paste Foto</button><input ref={p.filePartRef} type="file" accept="image/*" hidden onChange={(e)=>p.addPartsFromImage(e.target.files[0])}/><small style={{display:"block",marginTop:8}}>Tips: bisa juga copy gambar tabel part lalu klik area aplikasi dan tekan Ctrl+V.</small><hr/>
  <h3>🔨 Estimasi Sublet / Additional Job</h3><EditableSimple rows={f.sublets} type="sublets" updateRow={p.updateRow} deleteRow={p.deleteRow} cols={["subletNo","subletName","qty","price"]}/><button onClick={p.addSublet} style={styles.btn}>⊕ Add Sublet</button><hr/><h3>🧰 Jasa Pekerjaan</h3><div style={styles.flatRateBox}><b>Flat Rate</b><span>Pilih Type</span><select value={p.repairType || "Tidak Ada"} onChange={(e)=>p.handleFlatRateChange(e.target.value)} style={styles.select}>{p.flatRateOptions.map(x=><option key={x} value={x}>{x}</option>)}</select><span>Flat Rate</span><input readOnly value={rupiah(p.flatRateList[p.repairType] || 0)} style={styles.flatRateInput}/></div><EditableSimple rows={f.jasa} type="jasa" updateRow={p.updateRow} deleteRow={p.deleteRow} cols={["jasaName","rate","price"]} repairType={p.repairType}/><button onClick={p.addJasa} style={styles.btn}>⊕ Add Jasa</button><div style={styles.discount}><Input label="Disc Part (%)" value={f.discPartPct} onChange={(v)=>p.setField("discPartPct",v)}/><Input label="Disc Part (Rp)" value={f.discPartRp} onChange={(v)=>p.setField("discPartRp",toNumber(v))}/><Input label="Disc Jasa (%)" value={f.discJasaPct} onChange={(v)=>p.setField("discJasaPct",v)}/><Input label="Disc Jasa (Rp)" value={f.discJasaRp} onChange={(v)=>p.setField("discJasaRp",toNumber(v))}/><div style={styles.totalBox}><p>Total Parts<br/><b>Rp {rupiah(p.totals.totalParts)}</b></p><p>Total Sublet<br/><b>Rp {rupiah(p.totals.totalSublet)}</b></p><p>Total Jasa<br/><b>Rp {rupiah(p.totals.totalJasa)}</b></p><b>Grand Total</b><h2>Rp {rupiah(p.totals.grandTotal)}</h2></div></div><div style={styles.actions}><button onClick={p.resetForm} style={styles.red}>↻ Reset</button><button onClick={p.updateHargaSpareJasa} style={styles.btn}>↻ Update Harga Spare & Jasa</button><button onClick={p.saveMasterFromCurrent} style={styles.btn}>💾 Save Master</button><button onClick={p.saveHistory} style={styles.green}>Submit / Save History</button><button onClick={p.downloadPDF} style={styles.blue}>Submit & Create PDF</button></div></section><PDFView refx={p.pdfRef} form={f} totals={p.totals}/></> }

function SearchInput({label,value,onChange,onSelect,suggestions=[]}){
  const [open,setOpen]=useState(false);
  const q=norm(value);
  const items=(suggestions||[]).filter(x=>norm(x.value).includes(q)||norm(x.label).includes(q)).slice(0,30);
  return <label style={{...styles.label,position:"relative",display:"block"}}>{label}<input value={value||""} onChange={(e)=>{onChange(e.target.value);setOpen(true)}} onFocus={()=>setOpen(true)} onBlur={()=>setTimeout(()=>setOpen(false),180)} style={styles.input}/>{open&&items.length>0&&<div style={styles.suggestBox}>{items.map((x,i)=><div key={(x.value||"")+i} onMouseDown={(e)=>{e.preventDefault();onSelect?onSelect(x.value):onChange(x.value);setOpen(false)}} style={styles.suggestItem}>{x.label||x.value}</div>)}</div>}</label>
}
function SearchCell({value,onChange,onSelect,suggestions=[]}){
  const [open,setOpen]=useState(false);
  const q=norm(value);
  const items=(suggestions||[]).filter(x=>norm(x.value).includes(q)||norm(x.label).includes(q)).slice(0,30);
  return <div style={{position:"relative"}}><input value={value||""} onChange={(e)=>{onChange(e.target.value);setOpen(true)}} onFocus={()=>setOpen(true)} onBlur={()=>setTimeout(()=>setOpen(false),180)} style={styles.cellInput}/>{open&&items.length>0&&<div style={styles.suggestBox}>{items.map((x,i)=><div key={(x.value||"")+i} onMouseDown={(e)=>{e.preventDefault();onSelect?onSelect(x.value):onChange(x.value);setOpen(false)}} style={styles.suggestItem}>{x.label||x.value}</div>)}</div>}</div>
}
...
function History({history,editHistory,setHistory,newForm}){
  const [q, setQ] = useState("");
  const key = norm(q);
  const filtered = !key ? history : history.filter((h) => {
    const gabung = [
      h.polisi, h.customer, h.kendaraan, h.rangka, h.tahun, h.phone, h.contact, h.alamat,
      h.createdAt, h.updatedAt, h.grandTotal
    ].join(" ");
    return norm(gabung).includes(key);
  });

  return <section style={styles.card}>
    <button style={styles.greenRight} onClick={newForm}>+ Buat Estimasi Baru</button>
    <h1>History Estimasi</h1>
    <div style={{margin:"12px 0 16px"}}>
      <input
        value={q}
        onChange={(e)=>setQ(e.target.value)}
        placeholder="Cari history: plat nomor, nama customer, type kendaraan, no rangka, telp, alamat..."
        style={{...styles.input, maxWidth:650}}
      />
      <small style={{display:"block",marginTop:6}}>
        Hasil: {filtered.length} dari {history.length} data
      </small>
    </div>
    <table style={styles.table}>
      <thead><tr><th>No</th><th>No Polisi</th><th>Pelanggan</th><th>Type</th><th>Grand Total</th><th>Dibuat</th><th>Update</th><th>Action</th></tr></thead>
      <tbody>{filtered.length===0?<tr><td colSpan="8" style={{textAlign:"center",padding:30}}>Data tidak ditemukan.</td></tr>:filtered.map((h,i)=><tr key={h.id}><td>{i+1}</td><td>{h.polisi}</td><td>{h.customer}</td><td>{h.kendaraan}</td><td>Rp {rupiah(h.grandTotal)}</td><td>{h.createdAt}</td><td>{h.updatedAt}</td><td><button onClick={()=>editHistory(h)} style={styles.btn}>Edit</button><button onClick={()=>editHistory({...h,id:uid(),createdAt:new Date().toLocaleString("id-ID"),updatedAt:new Date().toLocaleString("id-ID")})} style={styles.btn}>Copy</button><button onClick={()=>setHistory(history.filter(x=>x.id!==h.id))} style={styles.del}>Hapus</button></td></tr>)}</tbody>
    </table>
  </section>
}
...
function MasterPart({partDb,setPartDb}){
  ...
  const updateHargaBulk = async () => {
    const rows = parseImportRows(bulkHarga);
    if (!rows.length) {
      alert("Paste data perubahan harga dulu. Format: Part No | Part Name | Price");
      return;
    }
    ...
    rows.forEach((cols) => {
      if (cols.length < 2) return;
      const [partNo, partNameOrPrice, priceRaw] = cols;
      if (/part\s*no/i.test(partNo)) return;

      const price = priceRaw !== undefined ? toNumber(priceRaw) : toNumber(partNameOrPrice);
      const partName = priceRaw !== undefined ? partNameOrPrice : "";
      if (!partNo || !price) return;

      const idx = next.findIndex((x) => norm(x.partNo) === norm(partNo));
      if (idx >= 0) {
        next[idx] = {
          ...next[idx],
          partNo,
          partName: partName || next[idx].partName || "-",
          price,
        };
      } else {
        next.push({ partNo, partName: partName || "-", price });
      }
      processed++;
    });
    ...
    try {
      const rowsForDb = next.filter((p) => p.partNo).map(partToDb);
      if (rowsForDb.length) {
        const { error } = await supabase.from("spareparts").upsert(rowsForDb, { onConflict: "part_no" });
        if (error) throw error;
      }
      alert(`${processed} harga sparepart diproses dan disimpan online.`);
    } catch (err) {
      console.error(err);
      alert(`${processed} harga diproses di aplikasi, tapi gagal simpan online. Cek RLS policy / CORS.`);
    }
  };
  ...
  const saveOnline = async () => {
    try {
      const rows = partDb.filter((p) => p.partNo).map(partToDb);
      if (rows.length) {
        const { error } = await supabase.from("spareparts").upsert(rows, { onConflict: "part_no" });
        if (error) throw error;
      }
      alert("Harga sparepart sudah disimpan/update ke Supabase.");
    } catch (err) {
      console.error(err);
      alert("Gagal update sparepart ke Supabase. Cek RLS policy / CORS.");
    }
  };
  const deleteAll = async () => {
    if (!confirm("Hapus semua master sparepart?")) return;
    setPartDb([]);
    localStorage.removeItem("sparepart_db_v5");
    try {
      const { error } = await supabase.from("spareparts").delete().neq("part_no", "__never__");
      if (error) throw error;
      alert("Semua master sparepart sudah dihapus dari aplikasi dan Supabase.");
    } catch (err) {
      console.error(err);
      alert("Sparepart di aplikasi sudah kosong, tapi gagal hapus di Supabase. Cek RLS policy / CORS.");
    }
  };
  return <section style={styles.card}><h1>Master Sparepart</h1>
    ...
    <button style={styles.btn} onClick={saveOnline}>Update Harga Online</button>
    <div style={{marginTop:18,padding:12,border:"1px solid #ddd",borderRadius:8,background:"#fafafa"}}>
      <b>Update Harga Sparepart</b>
      <p style={{margin:"6px 0 8px"}}>Paste dari Excel jika ada perubahan harga. Format: Part No | Part Name | Price, atau Part No | Price.</p>
      <textarea value={bulkHarga} onChange={(e)=>setBulkHarga(e.target.value)} placeholder={'Contoh:\n08880-85986\tTMO 10W-30 SN 1L\t120000\n15601-BZ030\t35000'} style={styles.bulkBox}/>
      <button style={styles.blue} onClick={updateHargaBulk}>Update Harga Sparepart</button>
    </div>
    <button style={styles.red} onClick={deleteAll}>Delete All Sparepart</button>
...
function MasterJasa({jasaDb,setJasaDb}){
  const [bulk, setBulk] = useState("");
  ...
  const importBulk = () => {
    ...
    rows.forEach((cols) => {
      if (cols.length < 2) return;
      const [kodeRaw, hargaRaw] = cols;
      if (/kode|type|jasa|harga|price/i.test(kodeRaw)) return;

      const kode = String(kodeRaw || "").trim().toUpperCase();
      const harga = toNumber(hargaRaw);
      if (!kode) return;

      const idx = next.findIndex((x) => String(x.kode || "").toUpperCase() === kode);
      const data = { kode, harga };
      if (idx >= 0) next[idx] = { ...next[idx], ...data };
      else next.push(data);
      processed++;
    });

    if (!next.some((x)=>x.kode === "Tidak Ada")) next.unshift({ kode:"Tidak Ada", harga:0 });
    setJasaDb(next);
    setBulk("");
    alert(`${processed} harga jasa diproses.`);
  };

  const exportJson = () => {
    const blob = new Blob([JSON.stringify(jasaDb, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob); const a = document.createElement("a");
    a.href = url; a.download = "master-jasa.json"; a.click(); URL.revokeObjectURL(url);
  };

  return <section style={styles.card}><h1>Master Jasa</h1>
    <p><b>Update harga jasa banyak:</b> copy dari Excel lalu paste di bawah. Format: Type/Kode | Harga.</p>
    <textarea value={bulk} onChange={(e)=>setBulk(e.target.value)} placeholder={'Contoh dari Excel:\nLGGC\t180930\nCOM\t223110\nECO\t256410'} style={styles.bulkBox}/>
    <button style={styles.green} onClick={importBulk}>Import / Update Harga Jasa</button>
    <button style={styles.btn} onClick={exportJson}>Export Backup</button>
    <button style={styles.btn} onClick={()=>setJasaDb([...jasaDb,{kode:"",harga:0}])}>+ Add Jasa</button>
    <table style={styles.table}><thead><tr><th>Type / Kode</th><th>Harga Jasa</th><th>Act</th></tr></thead><tbody>{jasaDb.map((j,i)=><tr key={i}><td><input value={j.kode} onChange={e=>setJasaDb(jasaDb.map((x,k)=>k===i?{...x,kode:e.target.value}:x))} style={styles.cellInput}/></td><td><input value={j.harga} onChange={e=>setJasaDb(jasaDb.map((x,k)=>k===i?{...x,harga:toNumber(e.target.value)}:x))} style={styles.cellInput}/></td><td><button style={styles.del} onClick={()=>setJasaDb(jasaDb.filter((_,k)=>k!==i))}>Hapus</button></td></tr>)}</tbody></table>
  </section>
}
...
function PDFView({refx,form,totals}){
  ...
  const noEstimasi = form.noEstimasi || `EST-${new Date().toISOString().slice(0,10).replace(/-/g,"")}`;
  ...
  return <div ref={refx} style={styles.pdfNew}>
    ...
    <div style={styles.pdfBlankColumn}><b>Catatan / Keterangan Tambahan:</b><br/>{form.catatan || ""}</div>
...
pdfBlankColumn:{border:"1px solid #333",minHeight:32,marginTop:12,marginBottom:6,padding:8,textAlign:"left",fontSize:10,lineHeight:1.25,whiteSpace:"pre-wrap"},
