...
function Estimasi(p){ const f=p.form; const customers = p.customerDb || [];
  const fillCustomer = (v) => {
    const plat = String(v || "").toUpperCase();
    const clean = (x) => String(x || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
    const q = clean(plat);
    p.setField("polisi", plat);
    if (!q) return;

    const found = customers.find((c) => clean(c.polisi) === q)
      || customers.find((c) => clean(c.polisi).startsWith(q))
      || customers.find((c) => clean(c.polisi).includes(q));

    if (found) {
      p.setField("polisi", found.polisi || plat);
      p.setField("kendaraan", found.kendaraan || "");
      p.setField("rangka", found.rangka || "");
      p.setField("tahun", found.tahun || "");
      p.setField("customer", found.customer || "");
      p.setField("contact", found.contact || "");
      p.setField("phone", found.phone || "");
      p.setField("alamat", found.alamat || "");
    }
  };
  return <><section style={styles.card}><h2 style={{textAlign:"center"}}>FORM ESTIMASI</h2><div style={styles.tabs}><b>Profile</b><b>Estimasi Biaya</b></div><h3>🚙 Informasi Kendaraan</h3><div style={styles.grid4}><Input label="No Polisi *" list="polisi-list" value={f.polisi} onChange={fillCustomer}/><Input label="Type Kendaraan *" list="kendaraan-list" value={f.kendaraan} onChange={(v)=>p.setField("kendaraan",v)}/><Input label="No Rangka *" list="rangka-list" value={f.rangka} onChange={(v)=>p.setField("rangka",v)}/><Input label="Tahun *" value={f.tahun} onChange={(v)=>p.setField("tahun",v)}/></div><datalist id="polisi-list">{customers.map((c,i)=><option key={(c.polisi||'')+i} value={c.polisi || ''}>{`${c.customer || ''} - ${c.kendaraan || ''} - ${c.rangka || ''}`}</option>)}</datalist><datalist id="kendaraan-list">{[...new Set(customers.map(c=>c.kendaraan).filter(Boolean))].map((x,i)=><option key={x+i} value={x}/>)}</datalist><datalist id="rangka-list">{customers.map((c,i)=><option key={(c.rangka||'')+i} value={c.rangka || ''}>{`${c.polisi || ''} - ${c.customer || ''} - ${c.kendaraan || ''}`}</option>)}</datalist><h3>👤 Informasi Pelanggan</h3><div style={styles.grid2}><Input label="Nama Pelanggan *" value={f.customer} onChange={(v)=>p.setField("customer",v)}/><Input label="No Telepon" value={f.phone} onChange={(v)=>p.setField("phone",v)}/><Input label="Contact Person" value={f.contact} onChange={(v)=>p.setField("contact",v)}/><Input label="Alamat" value={f.alamat} onChange={(v)=>p.setField("alamat",v)}/></div><h3>▣ Informasi Pembuat Estimasi</h3><div style={styles.grid3}><Input label="Nama Staff *" value={f.staff} onChange={(v)=>p.setField("staff",v)}/><Input label="Jabatan" value={f.jabatan} onChange={(v)=>p.setField("jabatan",v)}/><Input label="Kepala Bengkel" value={f.kepalaBengkel} onChange={(v)=>p.setField("kepalaBengkel",v)}/></div></section>
