...
function PDFView({refx,form,totals}){
  ...
  return <div ref={refx} style={styles.pdfNew}>
    <style>{`.pdfCellTable th,.pdfCellTable td{border:1px solid #222;padding:6px 7px;vertical-align:top}.pdfCellTable th{font-weight:700;text-align:center}.pdfCellTable td{text-align:center}`}</style>
    ...
    <table className="pdfCellTable" style={styles.pdfTableNew}>
      ...
      <tbody>{jasaSubletRows.length ? jasaSubletRows.map((r,i)=><tr key={i}><td>{i+1}</td><td style={{textAlign:"left"}}>{r.name}</td><td>{r.rate}</td><td>Rp {rupiah(r.price)}</td><td>Rp {rupiah(r.total)}</td></tr>) : <tr><td colSpan="5">-</td></tr>}</tbody>
    </table>
...
pdfNew:{width:820,background:"white",padding:28,margin:"30px auto",fontFamily:"Arial",color:"#111",fontSize:11},
pdfHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start"},pdfHeaderBar:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"14px 0 8px"},
pdfBranch:{fontSize:20,margin:"0 0 5px",fontWeight:"800",letterSpacing:.2},
pdfInfo:{margin:"3px 0",fontSize:9,fontWeight:"600"},
pdfLogo:{fontSize:26,fontWeight:"800",marginTop:10,letterSpacing:-1},
pdfDoubleLine:{borderTop:"4px double #111",margin:"18px 0 26px"},
pdfTitle:{textAlign:"center",fontSize:28,margin:"0 0 26px",fontWeight:"800"},pdfBlueTitle:{background:"#14284a",color:"white",textAlign:"center",fontWeight:"700",fontSize:11,padding:"6px",margin:"8px 0 10px"},
pdfDataGrid:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:35,marginBottom:18},pdfDataGridTop:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:45,marginBottom:16,fontSize:10},
pdfInfoRow:{display:"grid",gridTemplateColumns:"95px 8px 1fr",gap:3,margin:"3px 0",alignItems:"start"},
pdfSectionTitle:{textAlign:"center",fontSize:14,margin:"20px 0 8px",fontWeight:"800"},
pdfTableNew:{width:"100%",borderCollapse:"collapse",margin:"8px 0 12px",fontSize:11},
