...
function PDFView({refx,form,totals}){
  ...
  const discPartTampil = Math.max((totals.totalParts || 0) - (totals.totalPartsAfterDisc || 0), 0);
  const discJasaTampil = Math.max((totals.totalJasa || 0) - (totals.totalJasaAfterDisc || 0), 0);
  const discPartLabel = Number(form.discPartPct) ? `Disc Part (${form.discPartPct}%)` : "Disc Part";
  const discJasaLabel = Number(form.discJasaPct) ? `Disc Jasa (${form.discJasaPct}%)` : "Disc Jasa";
  const totalJasaSubletAfterDisc = (totals.totalJasaAfterDisc || 0) + (totals.totalSublet || 0);
  ...
  return <div ref={refx} style={styles.pdfNew}>
    ...
    <table className="pdfCellTable" style={styles.pdfTableNew}>
      <colgroup><col style={{width:"8%"}}/><col style={{width:"40%"}}/><col style={{width:"16%"}}/><col style={{width:"18%"}}/><col style={{width:"18%"}}/></colgroup>
      <thead><tr><th>No.</th><th>Sublet Pekerjaan</th><th>Qty</th><th>Price</th><th>Total</th></tr></thead>
    ...
    <div style={styles.pdfTotalRow}>
      <div><b>Total Parts</b><span>:</span><b>Rp {rupiah(totals.totalParts)}</b></div>
      <div><b>{discPartLabel}</b><span>:</span><b>Rp {rupiah(discPartTampil)}</b></div>
      <div><b>Total Jasa</b><span>:</span><b>Rp {rupiah(totals.totalJasa)}</b></div>
      <div><b>{discJasaLabel}</b><span>:</span><b>Rp {rupiah(discJasaTampil)}</b></div>
      <div><b>Total Sublet</b><span>:</span><b>Rp {rupiah(totals.totalSublet)}</b></div>
      <div><b>Total Jasa / Sublet</b><span>:</span><b>Rp {rupiah(totalJasaSubletAfterDisc)}</b></div>
    </div>
...
pdfHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start"},pdfHeaderBar:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"2px 0 10px",textAlign:"left"},
pdfBranch:{fontSize:23,margin:"0 0 4px",fontWeight:"900",letterSpacing:.05,textAlign:"left"},
pdfInfo:{margin:"3px 0",fontSize:9,fontWeight:"700",textAlign:"left"},
pdfLogo:{fontSize:25,fontWeight:"900",marginTop:3,letterSpacing:-1},
...
pdfDataGrid:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:35,marginBottom:18},pdfDataGridTop:{display:"grid",gridTemplateColumns:"1fr 1fr",columnGap:32,rowGap:0,marginBottom:12,fontSize:11,fontWeight:"700"},
pdfInfoRow:{display:"grid",gridTemplateColumns:"110px 8px 1fr",gap:3,margin:"5px 0",alignItems:"start",textAlign:"left",lineHeight:1.15},
...
pdfTotalRow:{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"7px 22px",borderTop:"2px solid #111",paddingTop:10,marginTop:12,fontSize:11.5},
